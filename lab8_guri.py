...
Each character appears only once in the result, preserving
the order of first occurrence.

E.g.,
>>> remove_duplicates_set("apple")
"aple"
>>> remove_duplicates_set("Popsipple")
"Popsile"
...
def gem_counting(stones, gems):
    """
    Count how many stones are actually gems.

Uses a set to check each stone against the gem types.

Parameters:
    stones (str): A string where each character represents a stone type.
    gems (str): A string where each character represents a gem type.
...
4
>>> gem_counting("bFfL", "cQqW")
0

    """
    types_of_gems = set(gems)
    counter = 0
    
    for s in stones:
        if s in types_of_gems:
            counter =+1
...
def students_id(ids):
    """
    Count the number of different students who use the shuttle service.

Uses a set to track unique student ids.

Parameters:
    ids (list): A list of student id strings.

Returns:
    int: The number of unique students.

E.g.,
>>> students_id(['002', '003', '001', '012', '003', '001'])
4
...
def students_id_occurrences(ids):
    """
    Count the occurrences of each student id in the list.

Parameters:
    ids (list): A list of student id strings.

Returns:
    dict: A dictionary mapping each student id to its occurrence count.
...
Uses regular expressions to extract words and a dictionary
to track their counts.

Parameters:
    paragraph (str): A string containing one or more sentences.

Returns:
    dict: A dictionary mapping each word to its frequency.
